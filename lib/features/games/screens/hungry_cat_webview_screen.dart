...
class _HungryCatWebviewScreenState extends State<HungryCatWebviewScreen>
  ...
  int  get _totalBetAmount =>
      _betsByFood.values.fold(0, (s, v) => s + v);
  ...
  void initState() {
    ...
    _pulseCtrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 350));
    _pulseAnim = Tween<double>(begin: 1.0, end: 1.42)
        .animate(CurvedAnimation(parent: _pulseCtrl, curve: Curves.easeInOut));

    _breathCtrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 2200))
      ..repeat(reverse: true);
    _breathAnim = Tween<double>(begin: 0.95, end: 1.05)
    ...
    _resultSlideCtrl = AnimationController(vsync: this, duration: const Duration(milliseconds: 380));
    _resultSlideAnim = Tween<Offset>(begin: const Offset(0, 0.3), end: Offset.zero)
  ...
  Future<void> _loadGame() async {
    ...
    try {
      ...
      _applyRound(results[2] as HungryCatGlobalRound);
      _subscribeToRound((results[2] as HungryCatGlobalRound).roundId);
    } catch (e, st) {
      debugError('[HungryCat] loadGame', e, st);
      if (!mounted) return;
      setState(() { _phase = _Phase.error; _errorMsg = _friendlyError('$e'); });
    }
  }
    ...
    _channel = SupabaseService.requiredClient
        ...
        .onPostgresChanges(
          ...
          filter: PostgresChangeFilter(
            type: PostgresChangeFilterType.eq, column: 'id', value: roundId,
          ),
  ...
  void _onRoundUpdate(PostgresChangePayload payload) {
    ...
    final mult     = row['winning_multiplier'] == null
        ? null : (row['winning_multiplier'] as num).toDouble();
  ...
  Future<void> _tapFood(int index) async {
    ...
    if (_balance < betAmount) {
      _showSnack(_ar ? 'رصيد غير كافٍ' : 'Insufficient coins', type: SroodToastType.warning);
      return;
    }
    ...
    debugPrint('[HungryCatBet] tap seq=$mySeq food=${food.foodId} roundId=$_roundId amount=$betAmount');
    debugPrint('[HungryCatPerf] tap received seq=$mySeq at ${tapAt.millisecondsSinceEpoch}ms');
    HapticFeedback.lightImpact();

    setState(() {
      _balance -= betAmount;
      _foodPendingCounts[food.foodId] = (_foodPendingCounts[food.foodId] ?? 0) + 1;
    });
    ...
    try {
      final result = await _service.placeGlobalBet(
        roundId: _roundId!, foodId: food.foodId, amount: betAmount,
      );
      if (!mounted) return;
      ...
      if (!mounted) return;
      setState(() { _balance += betAmount; _decrementPending(food.foodId); });

      if (errStr.contains('duplicate_bet')) { _refreshBalance(); return; }
      if (errStr.contains('betting_closed') || errStr.contains('betting_still_open')) return;
  ...
  Future<void> _triggerSettle() async {
    ...
    try {
      ...
      if (!mounted) return;
      _applyWinner(result.winningFoodId, result.winningFoodIcon,
          result.winningFoodName, result.winningMultiplier);
    } catch (e, st) {
    ...
    _resultPollTimer = Timer(Duration(milliseconds: delayMs), () async {
      ...
      try {
        ...
        if (round.isSettled && _winFoodId == null) {
          _applyWinner(round.winningFoodId, round.winningFoodIcon,
              round.winningFoodName, round.winningMultiplier);
          return;
        }
      } catch (e) {
  ...
  void _applyWinner(String? foodId, String? foodIcon, String? foodName, double? mult) {
    _winFoodId   = foodId;
  ...
  Future<void> _runFreeSpin() async {
    ...
    _service.fetchCoinBalance().then((newBalance) {
      ...
      setState(() { _balance = newBalance; _spinDelta = delta; });
    }).catchError((Object e) { debugPrint('[HungryCat] balance fetch: $e'); });
  ...
  String _multLabelAt(int i) {
    ...
    final v = mult.truncateToDouble() == mult
        ? mult.toInt().toString() : mult.toStringAsFixed(1);
    return '${v}x';
  }

  String _emojiForFoodId(String foodId) {
    for (final f in _foods) { if (f.foodId == foodId) return f.icon; }
    return '🍽️';
  }
    ...
    return await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Text(
          ...
          style: const TextStyle(color: _kBrown, fontWeight: FontWeight.w900),
        ),
        content: Text(
          _ar ? 'الرهانات المعلقة ستظل صالحة.' : 'Your pending bets remain valid.',
          style: const TextStyle(color: _kWoodMed),
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(false),
            child: Text(_ar ? 'البقاء' : 'Stay',
                style: const TextStyle(color: _kWoodMed)),
          ),
          TextButton(
            onPressed: () => Navigator.of(context).pop(true),
            child: Text(_ar ? 'مغادرة' : 'Leave',
                style: const TextStyle(color: _kRedRound, fontWeight: FontWeight.w800)),
          ),
        ],
      ),
    ) ?? false;
      ...
      child: Row(
        children: [
          ...
          GestureDetector(
            onTap: () {
              HapticFeedback.lightImpact();
              setState(() { _globalMuted = !_globalMuted; _sounds.muted = _globalMuted; });
            },
            child: Container(
              width: 36, height: 36,
              decoration: BoxDecoration(
          ...
          Container(
            ...
            child: Text(
              _ar ? 'القواعد >' : 'RULE >',
              style: const TextStyle(color: _kBrown, fontSize: 12, fontWeight: FontWeight.w800),
            ),
          ),
        ],
      ),
  ...
  Widget _positionedBubble(int i, int n, Offset center, double radius, double bubSize) {
    ...
    final isWinner   = _phase == _Phase.settled && _winFoodId != null && i == _findWinnerIndex();
    ...
    if (_phase == _Phase.settled && _winFoodId != null && !isWinner) opacity = 0.30;
    ...
    Widget bubble = AnimatedBuilder(
      ...
      child: Semantics(
        ...
        child: _HungryCatBubble(
          emoji: _emojiAt(i), multLabel: _multLabelAt(i), size: bubSize,
          isActive: isActive, isWinner: isWinner, isSelected: isSelected,
          isBusy: isBusy, betAmount: betAmount, formatCoins: _formatCoins,
          isArabic: _ar, isSpinning: isSpinning,
        ),
      ),
    );

    if (canTap) bubble = GestureDetector(onTap: () => _tapFood(i), child: bubble);

    return Positioned(
      left: cx - bubSize / 2, top: cy - bubSize / 2,
      width: bubSize, height: bubSize,
  ...
  Widget _buildCatCenter(double size) {
    ...
    final String catEmoji = spinning ? '😸' : settled ? '😻' : '🐱';
    final bool   urgent   = betting && _secsLeft <= 5;

    return AnimatedBuilder(
      animation: _breathAnim,
      builder: (_, child) => Transform.scale(
        scale: spinning ? 1.0 : _breathAnim.value, child: child,
      ),
      child: Container(
        width: size, height: size,
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: urgent ? _kGoldCoin : _kWoodDark,
            width: 3,
          ),
          boxShadow: [
            BoxShadow(
              color: (spinning ? _kGoldCoin : Colors.black).withValues(alpha: 0.3),
              ...
              Column(
                children: [
                  if (_waitingForResult)
                    _CenterLabel(text: _ar ? 'جارٍ الاحتساب...' : 'The result is\ncoming', size: size)
                  else
                    _CenterLabel(text: _ar ? 'جارٍ الدوران...' : 'Spinning...', size: size),
                ],
              )
      ...
      child: FadeTransition(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            color: _kCream,
            border: Border.all(
              color: won ? _kGreenWin : _kWoodMed, width: 2,
            ),
            boxShadow: [
              BoxShadow(
            ...
            children: [
              ...
              Column(
                ...
                children: [
                  Text(
                    name,
                    style: const TextStyle(
                      color: _kBrown, fontWeight: FontWeight.w900, fontSize: 15,
                    ),
                  ),
                  if (mult.isNotEmpty)
                    Text(
                      mult,
                      style: const TextStyle(
                        color: _kGoldCoin, fontWeight: FontWeight.w900, fontSize: 18,
                      ),
                    ),
                ],
              ),
              if (_hasBets && _spinDelta != null) ...[
                ...
                Container(
                  ...
                  child: Text(
                    won
                        ? (_ar ? '+${_formatCoins(_spinDelta!)} 🪙' : '+${_formatCoins(_spinDelta!)} 🪙')
                        : (_ar ? '-${_formatCoins(_totalBetAmount)} 🪙' : '-${_formatCoins(_totalBetAmount)} 🪙'),
                    style: const TextStyle(
            ...
            child: Row(
              ...
              children: [
                ...
                Text(
                  _ar ? 'تخطي' : 'Skip',
                  style: const TextStyle(
                    color: _kBrown, fontSize: 12, fontWeight: FontWeight.w800,
                  ),
                ),
              ],
            ),
      ...
      child: ListView.builder(
        ...
        itemBuilder: (ctx, i) {
          ...
          return Container(
            ...
            decoration: BoxDecoration(
              shape: BoxShape.circle,
              color: big ? _kGoldCoin.withValues(alpha: 0.25) : _kCream.withValues(alpha: 0.5),
              border: Border.all(
                color: big ? _kGoldCoin : _kWoodMed.withValues(alpha: 0.5),
              ),
            ),
            child: Center(child: Text(h.foodIcon, style: const TextStyle(fontSize: 18))),
          );
        },
      ),
    );
        ...
        children: [
          ...
          if (_betsByFood.isEmpty && _foodPendingCounts.isEmpty && active)
            Padding(
              padding: const EdgeInsets.only(bottom: 6),
              child: Text(
                _ar ? 'اختر مبلغ الرهان ثم اضغط على الطعام 👆' : 'Choose wager amount > choose food 👆',
                style: TextStyle(
  ...
  Widget _buildBetStatus() {
      ...
      return Row(
        ...
        children: [
          ...
          Text(
            ...
            style: const TextStyle(color: _kCream, fontSize: 12, fontWeight: FontWeight.w700),
          ),
        ],
      );
    }
    ...
    if (_betsByFood.isNotEmpty) {
      return Wrap(
        ...
        children: _betsByFood.entries.map((e) => Container(
          ...
          child: Text(
            ...
            style: const TextStyle(color: _kCream, fontSize: 11, fontWeight: FontWeight.w700),
          ),
        )).toList(),
      );
    }
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                gradient: RadialGradient(
                  colors: sel && !locked
                      ...
                      : disabled
                          ...
                          : [const Color(0xFFE8A830), _kGoldCoin, const Color(0xFFC07810)],
                ),
                border: Border.all(
                ...
                boxShadow: sel && !locked
                    ? [BoxShadow(color: _kGoldCoin.withValues(alpha: 0.6), blurRadius: 10, spreadRadius: 1)]
              ...
              child: Column(
                ...
                children: [
                  Text(
                    '❌',
                    style: TextStyle(fontSize: disabled ? 10 : 11),
                  ),
                  Text(
                    _formatCoins(chip),
                    style: TextStyle(
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  const SizedBox(
                    width: 28, height: 28,
                    child: CircularProgressIndicator(color: _kWoodMed, strokeWidth: 2.5),
                  ),
                  const SizedBox(height: 12),
                  Text(
                    _ar ? 'إعادة الاتصال...' : 'Reconnecting…',
                    style: const TextStyle(
                      color: _kBrown, fontSize: 15, fontWeight: FontWeight.w800,
                    ),
                  ),
                ],
              ),
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            ...
            style: const TextStyle(
              color: _kCream, fontSize: 16, fontWeight: FontWeight.w800,
            ),
          ),
        ],
      ),
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              label,
              textAlign: TextAlign.center,
              style: const TextStyle(color: _kWoodMed, fontSize: 9, fontWeight: FontWeight.w600),
            ),
          ],
        ),
      ),
...
class _HungryCatBubble extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final raw   = multLabel.endsWith('x') ? multLabel.substring(0, multLabel.length - 1) : multLabel;
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: shadowBlur > 0
            ? [BoxShadow(color: shadowColor, blurRadius: shadowBlur, spreadRadius: spreadR)]
        ...
        child: AnimatedContainer(
          ...
          child: Stack(
            ...
            children: [
              ...
              if (isWinner)
                Positioned.fill(
                  child: ClipOval(child: _ShimmerOverlay()),
                ),
              ...
              if (isBusy)
                SizedBox(
                  ...
                  child: const CircularProgressIndicator(
                    strokeWidth: 2.5, color: _kGreenWin,
                  ),
                ),
              ...
              if (betAmount > 0)
                Positioned(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      gradient: const LinearGradient(colors: [_kGoldLight, _kGoldCoin]),
                ...
                Positioned(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      borderRadius: BorderRadius.circular(10),
                      boxShadow: [
                        BoxShadow(color: _kGreenWin.withValues(alpha: 0.7), blurRadius: 8, spreadRadius: 1),
                      ],
                    ),
                    child: Text(
                      isArabic ? '✨ فاز!' : '✨ WIN!',
...
class _ShimmerPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRect(
      ...
      Paint()..shader = gradient.createShader(Rect.fromLTWH(x, 0, size.width, size.height)),
    );
  }
...
class _CoinBurstPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final paint = Paint()..color = _kGoldCoin.withValues(alpha: (1 - ease) * 0.9);
...
class _WheelFramePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(
      center, radius - size.width * 0.014,
      Paint()
    ...
    for (int i = 0; i < n; i++) {
      ...
      canvas.drawCircle(Offset(x, y), 3, Paint()..color = _kCream.withValues(alpha: 0.8));
    }
  }
...
class _HungryCatSounds {
  ...
  Future<void> init() async {
    ...
    await _tryLoad(_event, 'assets/sounds/hcat_bet.wav',
  ...
  void playBetPlaced() => _playEvent('assets/sounds/hcat_bet.wav',
      fallback: 'assets/sounds/lucky_bag_open.mp3');
  void playSpinStart() => _playEvent('assets/sounds/hcat_spin.wav',
      fallback: 'assets/sounds/lucky_bag_open.mp3');
  void playWin()       => _playEvent('assets/sounds/hcat_win.wav',
